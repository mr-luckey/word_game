...
class AuthService {
  ...
  /// Waits for Firebase to emit the restored session (stay logged in after app close).
  Future<void> waitForPersistedSession() async {
    await _auth.authStateChanges().first;
    if (_auth.currentUser != null) {
      try {
        await _auth.currentUser!.getIdToken(false);
      } catch (_) {}
      return;
    }

    if (_prefs.getString(_authProviderKey) != 'google') return;
  ...
  Future<void> signUpWithEmail({
    ...
    await _prefs.setString(_authProviderKey, 'email');
  }
  ...
  Future<void> signInWithGoogle() async {
    ...
    final name = user.displayName?.trim().isNotEmpty == true
        ...
        : 'Player';
    await _prefs.setString(_displayNameKey, name);
    await _prefs.setString(_authProviderKey, 'google');
  }
